...
void GrGLCaps::init(const GrGLContextInfo& ctxInfo)
{
	...
	fDiscardFBSupport = true; // ctxInfo.hasExtension("GL_EXT_discard_framebuffer");

	fVertexArrayObjectSupport = true;
    ...
    if (numFormats)
	{
        SkAutoSTMalloc<10, GLint> formats(numFormats);
		glGetIntegerv(GL_COMPRESSED_TEXTURE_FORMATS, formats);
        for (int i = 0; i < numFormats; ++i) {
			if (formats[i] == GL_PALETTE8_RGBA8_OES ) 
			{
	...
	fPathRenderingSupport = 0;
	fDstReadInShaderSupport = kNone_FBFetchType != fFBFetchType;

    // Disable scratch texture reuse on Mali and Adreno devices
	fReuseScratchTextures = true;
    //fReuseScratchTextures = kARM_GrGLVendor != ctxInfo.vendor() && kQualcomm_GrGLVendor != ctxInfo.vendor();
    ...
    if (GrGLCaps::kNone_MSFBOType != fMSFBOType) {
...
bool GrGLCaps::readPixelsSupported( GLenum format,
                                   GLenum type) const {
    ...
    GLint otherFormat = GL_RGBA;
    GLint otherType = GL_UNSIGNED_BYTE;
    ...
    return (GLenum)otherFormat == format && (GLenum)otherType == type;
}
