...
static uint32_t* lu2 = 0;
static uint32_t* lu3 = 0;
...
inline void generate_look_ups() {
	uint32_t i = 0;
	lu2 = (uint32_t *)malloc(sizeof(uint32_t) * 4);
	lu3 = (uint32_t *)malloc(sizeof(uint32_t) * 8);
	lu4 = (uint32_t *)malloc(sizeof(uint32_t) * 16);
	lu5 = (uint32_t *)malloc(sizeof(uint32_t) * 32);
	lu6 = (uint32_t *)malloc(sizeof(uint32_t) * 64);

	for(i = 0; i < 4 ; i++) {
		lu2[i] = (uint32_t) i / 3.0f * 255;
		lu3[i] = (uint32_t) i / 7.0f * 255;
...
//			return ((color & 0xe0)>>5<<6<<24) | ((color & 0x1c)>>2<<6<<16) | ((color & 0x03)<<7<<8) | 0x000000ff;
		case GDX2D_FORMAT_LUMINANCE_ALPHA: 
			return ((color & 0xff00) << 16) | ((color & 0xff00) << 8) | (color & 0xffff);
		case GDX2D_FORMAT_RGB888:
			return (color << 8) | 0x000000ff;
//			return (color<<24) | (color<<8 & 0x00ff0000) | (color>>8 & 0x0000ff00) | 0x000000ff;
		case GDX2D_FORMAT_RGBA8888:
			return color;
		case GDX2D_FORMAT_RGB565:
		...
		case GDX2D_FORMAT_RGBA4444:
			...
			return r | g | b | a;
//zhangchen extend
		case GDX2D_FORMAT_RGB332:
//			r = ((color & 0xe0)>>5)*36<<24;
//			g = ((color & 0x1c)>>2)*36<<16;
//			b = (color & 0x03)*85<<8;
			r = lu3[(color & 0xe0) >> 5] << 24;
			g = lu3[(color & 0x1c) >> 2] << 16;
			b = lu2[(color & 0x03)] << 8;
			a = color ? 0x000000ff : 0x00000000 ;
			return r | g | b | a;
		case GDX2D_FORMAT_GRAY2:
			return (color & 0x01) ? 0xffffffff : 0x00000000 ;
		default: 
			return 0;
	}
}
...
inline void set_pixel_RGB332(unsigned char *pixel_addr, uint32_t color) {
	*(uint8_t*)pixel_addr = (uint8_t)color;
}

inline void set_pixel_gray2(unsigned char *pixel_addr, uint32_t color) {
	*(uint8_t*)pixel_addr |= color ? 0xf0 : 0x00;
}
...
void blit_same_size(const gdx2d_pixmap* src_pixmap, const gdx2d_pixmap* dst_pixmap, 
	...
	for(;sy < src_y + height; sy++, dy++) {
		...
		for(sx = src_x, dx = dst_x; sx < src_x + width; sx++, dx++) {
			...
			unsigned char* src_ptr = src_pixmap->pixels + sx * sbpp + sy * spitch;
			unsigned char* dst_ptr = dst_pixmap->pixels + dx * dbpp + dy * dpitch;
			uint32_t src_col = to_RGBA8888(src_pixmap->format, pget(src_ptr));

			if(gdx2d_blend) {
				uint32_t dst_col = to_RGBA8888(dst_pixmap->format, dpget(dst_ptr));
...
void blit_bilinear(const gdx2d_pixmap* src_pixmap, const gdx2d_pixmap* dst_pixmap,
	...
	for(;i < dst_height; i++) {
		...
		for(j = 0 ;j < dst_width; j++) {
			...
			c1 = to_RGBA8888(src_pixmap->format, pget(src_ptr));
			if(sx + 1 < src_width)
				c2 = to_RGBA8888(src_pixmap->format, pget((src_ptr + sbpp)));
			else
				c2 = c1;
			if(sy + 1< src_height)
				c3 = to_RGBA8888(src_pixmap->format, pget((src_ptr + spitch))); 
			else
				c3 = c1;
			if(sx + 1< src_width && sy + 1 < src_height)
				c4 = to_RGBA8888(src_pixmap->format, pget((src_ptr + spitch + sbpp)));
			else 
				c4 = c1;
