...
static const size_t kInitialVectorSize = 4;
...
private:
	VectorBufferBase(const VectorBufferBase&);
	VectorBufferBase& operator=(const VectorBufferBase&);
public:
...
private:
	VectorBuffer(const VectorBuffer&);
...
public:
	...
	void reallyDeallocateBuffer(T* bufferToDeallocate)
	{
		Allocator::backingFree(bufferToDeallocate);
	}
...
private:
	...
	AlignedBuffer<m_inlineBufferSize, __alignof(T)> m_inlineBuffer;
};
...
template<typename T, size_t inlineCapacity, typename Allocator> template<typename U>
void Vector<T, inlineCapacity, Allocator>::append(const U& val)
{
...
template<typename T, size_t inlineCapacity, typename Allocator> template<typename U>
void Vector<T, inlineCapacity, Allocator>::appendSlowCase(const U& val)
{
...
template<typename T, size_t inlineCapacity, typename Allocator> template<typename U>
void Vector<T, inlineCapacity, Allocator>::uncheckedAppend(const U& val)
{
	ASSERT(size() < capacity());
	const U* ptr = &val;
	new (NotNull, end()) T(*ptr);
