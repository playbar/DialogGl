...
bool SkCanvas::translate(float dx, float dy) {
    fDeviceCMDirty = true;
    fCachedLocalClipBoundsDirty = true;
    return fMCRec->fMatrix->preTranslate(dx, dy);
}

bool SkCanvas::scale(float sx, float sy) {
    fDeviceCMDirty = true;
    fCachedLocalClipBoundsDirty = true;
    return fMCRec->fMatrix->preScale(sx, sy);
}

bool SkCanvas::rotate(float degrees) {
    fDeviceCMDirty = true;
    fCachedLocalClipBoundsDirty = true;
    return fMCRec->fMatrix->preRotate(degrees);
}

bool SkCanvas::skew(float sx, float sy) {
    fDeviceCMDirty = true;
    fCachedLocalClipBoundsDirty = true;
...
void SkCanvas::drawBitmap(const SkBitmap& bitmap, float x, float y,
                          const SkPaint* paint) {
...
void SkCanvas::DrawTextDecorations(const SkDraw& draw, const SkPaint& paint,
    ...
    if (flags & (SkPaint::kUnderlineText_Flag |
        ...
        float height = SkScalarMul(textSize, kStdUnderline_Thickness);
        SkRect   r;
        ...
        if (flags & SkPaint::kUnderlineText_Flag) {
            float offset = SkScalarMulAdd(textSize, kStdUnderline_Offset,
                                             start.fY);
            r.fTop = offset;
        ...
        if (flags & SkPaint::kStrikeThruText_Flag) {
            float offset = SkScalarMulAdd(textSize, kStdStrikeThru_Offset,
                                             start.fY);
            r.fTop = offset;
            r.fBottom = offset + height;
            DrawRect(draw, paint, r, textSize);
...
void SkCanvas::drawPoint(float x, float y, const SkPaint& paint) {
    SkPoint pt;
...
void SkCanvas::drawPoint(float x, float y, SkColor color) {
    SkPoint pt;
    SkPaint paint;
...
void SkCanvas::drawLine(float x0, float y0, float x1, float y1,
                        const SkPaint& paint) {
    SkPoint pts[2];
...
void SkCanvas::drawRectCoords(float left, float top,
...
void SkCanvas::drawCircle(float cx, float cy, float radius,
                          const SkPaint& paint) {
    if (radius < 0) {
        radius = 0;
    }
...
void SkCanvas::drawRoundRect(const SkRect& r, float rx, float ry,
                             const SkPaint& paint) {
    if (rx > 0 && ry > 0) {
        if (paint.canComputeFastBounds()) {
            SkRect storage;
...
void SkCanvas::drawArc(const SkRect& oval, float startAngle,
